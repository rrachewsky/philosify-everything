// V2AudioBar - v2 TTS audio bar (music mockup .audio anatomy) wired to
// services/ttsCache.js. Lazy generation: first click triggers preload,
// playback starts when the audio is ready. Speed cycles the same options
// as the legacy ListenButton; the hairline doubles as a seekable progress.
...
export function V2AudioBar({ result }) {
  ...
  const playUrl = useCallback(
    (url) => {
      ...
      audio.onerror = () => {
        setError(t('v2.music.audioPlaybackFailed', 'Audio playback failed'));
        setPlaying(false);
      };
      audio.ontimeupdate = () => {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      ...
      if (status === 'ready') {
        ...
        setError(
          err === 'timeout'
            ? t('v2.music.audioTimeout', 'Audio timed out — try again')
            : t('v2.music.audioFailed', 'Audio unavailable — try again')
        );
      }
    }, 400);
  ...
  const label = error
    ? error
    : preparing
      ? `${t('v2.music.preparingAudio', 'Preparing audio')} · ${fmt(elapsed)}`
      : t('v2.music.listen', 'Listen to the analysis');
  ...
  return (
    <div className="audio">
      <button
        ...
        aria-label={
          preparing
            ? t('v2.music.cancel', 'Cancel')
            : playing
              ? t('v2.music.pause', 'Pause')
              : t('v2.music.play', 'Play')
        }
      >
        {preparing ? '✕' : playing ? '❚❚' : '▶'}
      </button>
      ...
      <span
        ...
        ref={lineRef}
        onClick={handleSeek}
        aria-label={t('v2.music.seek', 'Seek')}
      >
        <i style={{ width: `${Math.round(progress * 100)}%` }} />
      </span>
      <button className="aspeed" onClick={handleSpeed}>
        {t('v2.music.speed', 'Speed')} {speed}x
      </button>
    </div>
  );
}
