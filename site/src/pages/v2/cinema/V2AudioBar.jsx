// V2AudioBar (Cinema) - v2 TTS audio bar (music-template .audio anatomy)
...
export function V2AudioBar({ result }) {
  ...
  const playUrl = useCallback(
    (url) => {
      ...
      audio.onerror = () => {
        setError(t('v2.cinema.audioPlaybackFailed', 'Audio playback failed'));
        setPlaying(false);
      };
      audio.ontimeupdate = () => {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      ...
      if (status === 'ready') {
        ...
        setError(
          err === 'timeout'
            ? t('v2.cinema.audioTimeout', 'Audio timed out — try again')
            : t('v2.cinema.audioFailed', 'Audio unavailable — try again')
        );
      }
    }, 400);
  ...
  const label = error
    ? error
    : preparing
      ? `${t('v2.cinema.preparingAudio', 'Preparing audio')} · ${fmt(elapsed)}`
      : t('v2.cinema.listen', 'Listen to the analysis');
  ...
  return (
    <div className="audio">
      <button
        ...
        aria-label={
          preparing
            ? t('v2.cinema.cancel', 'Cancel')
            : playing
              ? t('v2.cinema.pause', 'Pause')
              : t('v2.cinema.play', 'Play')
        }
      >
        {preparing ? '✕' : playing ? '❚❚' : '▶'}
      </button>
      ...
      <span
        ...
        ref={lineRef}
        onClick={handleSeek}
        aria-label={t('v2.cinema.seek', 'Seek')}
      >
        <i style={{ width: `${Math.round(progress * 100)}%` }} />
      </span>
      <button className="aspeed" onClick={handleSpeed}>
        {t('v2.cinema.speed', 'Speed')} {speed}x
      </button>
    </div>
  );
}
