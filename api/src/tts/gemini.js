...
const PODCAST_PHRASES_BOOK = {
  ...
  ja: {
    fascinating: "これは魅力的な本です。",
    interesting: "はい、この本には探求すべき本当に興味深い哲学的な側面があります。",
    historianHandoff: "{name}さん、この本の背景について教えてください。",
    background: "まず背景から始めましょう。この本の物語は何ですか？",
    verdictHandoff: "{name}さん、この本に対する最終評価は何ですか？",
  },
  ko: {
    fascinating: "정말 매력적인 책입니다.",
    interesting: "네, 이 책은 탐구할 가치가 있는 흥미로운 철학적 차원이 있습니다.",
    historianHandoff: "{name}님, 이 책의 역사적 배경에 대해 말씀해 주세요.",
    background: "먼저 배경부터 시작해 볼까요. 이 책의 이야기는 무엇인가요?",
    verdictHandoff: "{name}님, 이 책에 대한 최종 평가는 무엇인가요?",
  },
  zh: {
    fascinating: "这是一本引人入胜的书。",
    interesting: "是的，这本书有一些非常值得探讨的哲学维度。",
    historianHandoff: "{name}，请给我们讲讲这本书的历史背景。",
    background: "让我们从背景开始。这本书的故事是什么？",
    verdictHandoff: "{name}，你对这本书的最终评价是什么？",
  },
  ru: {
    fascinating: "Это увлекательная книга.",
    interesting: "Да, у этой книги есть действительно интересные философские аспекты для исследования.",
    historianHandoff: "{name}, расскажите нам об истории этой книги.",
    background: "Давайте начнём с предыстории. Какова история этой книги?",
    verdictHandoff: "{name}, каков ваш окончательный вердикт об этой книге?",
  },
  ar: {
    fascinating: "هذا كتاب رائع.",
    interesting: "نعم، هذا الكتاب له أبعاد فلسفية مثيرة للاهتمام تستحق الاستكشاف.",
    historianHandoff: "{name}، أخبرنا عن تاريخ هذا الكتاب.",
    background: "لنبدأ ببعض المعلومات الأساسية. ما قصة هذا الكتاب؟",
    verdictHandoff: "{name}، ما حكمك النهائي على هذا الكتاب؟",
  },
  he: {
    fascinating: "זה ספר מרתק.",
    interesting: "כן, לספר הזה יש ממדים פילוסופיים מעניינים באמת לחקור.",
    historianHandoff: "{name}, ספר לנו על ההיסטוריה של הספר הזה.",
    background: "בואו נתחיל עם קצת רקע. מה הסיפור מאחורי הספר הזה?",
    verdictHandoff: "{name}, מה פסק הדין הסופי שלך על הספר הזה?",
  },
  hi: {
    fascinating: "यह एक आकर्षक किताब है।",
    interesting: "हाँ, इस किताब में कुछ वाकई दिलचस्प दार्शनिक आयाम हैं जिन्हें खोजना चाहिए।",
    historianHandoff: "{name}, हमें इस किताब के इतिहास के बारे में बताइए।",
    background: "आइए कुछ पृष्ठभूमि से शुरू करें। इस किताब की कहानी क्या है?",
    verdictHandoff: "{name}, इस किताब पर आपका अंतिम फैसला क्या है?",
  },
  nl: {
    fascinating: "Dit is een fascinerend boek.",
    interesting: "Ja, dit boek heeft echt interessante filosofische dimensies om te verkennen.",
    historianHandoff: "{name}, vertel ons over de geschiedenis achter dit boek.",
    background: "Laten we beginnen met wat achtergrond. Wat is het verhaal achter dit boek?",
    verdictHandoff: "{name}, wat is je uiteindelijke oordeel over dit boek?",
  },
  pl: {
    fascinating: "To fascynujaca ksiazka.",
    interesting: "Tak, ta ksiazka ma naprawde interesujace wymiary filozoficzne warte zbadania.",
    historianHandoff: "{name}, opowiedz nam o historii tej ksiazki.",
    background: "Zacznijmy od kontekstu. Jaka jest historia tej ksiazki?",
    verdictHandoff: "{name}, jaki jest twoj ostateczny werdykt w sprawie tej ksiazki?",
  },
  tr: {
    fascinating: "Bu etkileyici bir kitap.",
    interesting: "Evet, bu kitabin kesfedilmeye deger gercekten ilginc felsefi boyutlari var.",
    historianHandoff: "{name}, bize bu kitabin tarihcesini anlat.",
    background: "Biraz arka planla baslayalim. Bu kitabin hikayesi ne?",
    verdictHandoff: "{name}, bu kitap hakkindaki nihai kararin ne?",
  },
  hu: {
    fascinating: "Ez egy lenyugozo konyv.",
    interesting: "Igen, ennek a konyvnek vannak igazan erdekes filozofiai dimenzioi.",
    historianHandoff: "{name}, meselj nekunk a konyv torteneterol.",
    background: "Kezdjuk egy kis hatterrel. Mi a tortenet e konyv mogott?",
    verdictHandoff: "{name}, mi a vegso iteleted errol a konyvrol?",
  },
  fa: {
    fascinating: "این کتاب جذابی است.",
    interesting: "بله، این کتاب ابعاد فلسفی واقعاً جالبی برای بررسی دارد.",
    historianHandoff: "{name}، درباره تاریخچه این کتاب به ما بگو.",
    background: "بیایید با کمی پیش‌زمینه شروع کنیم. داستان این کتاب چیست؟",
    verdictHandoff: "{name}، حکم نهایی تو درباره این کتاب چیست؟",
  },
};
